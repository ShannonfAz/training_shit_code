...
def add_motion_features(seq):
    ...
    输出: features (numpy array, shape = (64, 3))
          包含 [dx, dy, ddir_x]  只保留位移和方向变化的x分量
    """
    # 位移 (dx, dy)
    ...
    # 方向变化 (ddir_x, ddir_y)
    dir_diff = np.diff(dir_vec, axis=0)             # (63, 2)
    dir_diff = np.vstack([dir_diff, dir_diff[-1]])  # (64, 2)

    # 仅保留 dx, dy, ddir_x
    features = np.column_stack([diff, dir_diff[:, 0]])   # (64, 3)
    return features

def compute_feature_stats(sequences):
    """计算训练集上所有特征维度的均值和标准差（自动适应新特征数）"""
    all_feats = []
    for seq in sequences:
        feat = add_motion_features(seq)
        all_feats.append(feat)
    all_feats = np.concatenate(all_feats, axis=0)   # (N*64, 3)
    mean = all_feats.mean(axis=0)
    std = all_feats.std(axis=0) + 1e-8
    return mean, std

# -------------------- 数据集（不变，feature_mean/std会自动为3维） --------------------
class RoadShapeFixedLenDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        feat = add_motion_features(seq)                 # 现在返回 (64,3)
...
class GRUFixedLenClassifier(nn.Module):
    def __init__(self, input_dim=3, hidden_dim=16, num_layers=1, num_classes=6, dropout=0.2):
        super().__init__()
...
def train_model(model, train_loader, val_loader, epochs=100, lr=5e-4, device='cpu',
                class_weights=None, use_focal=False, gamma=1.0):
...
def main():
    ...
    feat_mean, feat_std = compute_feature_stats(X_train_raw)   # 现在返回 (3,) 的统计量
    print(f"特征均值: {feat_mean}, 特征标准差: {feat_std}")
    ...
    sqrt_weights = sqrt_weights / sqrt_weights.sum() * len(class_counts)
    ...
    trained_model = train_model(
        model, train_loader, val_loader, epochs=EPOCHS*2, lr=LR/16.0, device=device,
        class_weights=class_weights, use_focal=True, gamma=1.0
    )
